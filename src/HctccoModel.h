...
#error Recursive header files inclusion detected in HctccoModel.h
...
// class HctccoModel
/**
 * Description of class 'HctccoModel' <p>
 *
...
public:
    ...
    // Global biological parameters
        ...
        // my_rootRadiusMax: absolute cap on the root radius used to anchor the
        // tree to a patient/literature scale. Default = literature coronary
        // order-11 radius (3.176 mm diameter / 2). Set <=0 to disable the cap
        // and let Poiseuille set the root radius freely.
        double my_rootRadiusMax = 1.588;

        // my_termFlowCV: coefficient of variation of per-terminal perfusion
        // flow. 0 => uniform terminal flow (classic CCO, symmetric tree). >0
        // draws each terminal's flow from a lognormal with this CV, modelling
        // unequal perfusion territories. This injects the branching asymmetry
        // required to reproduce the coronary Strahler diameter ratio (~1.8).
        double my_termFlowCV = 0.0;

        // my_sideBranchBias: topological asymmetry control. 0 => pure
        // minimum-volume CCO (tends to build a balanced/symmetric tree, low
        // Strahler diameter ratio ~1.4). >0 multiplies the volume cost of a
        // candidate bifurcation by (Q_target/q_term)^(-bias), so attaching a
        // small new terminal as a SIDE BRANCH on a large high-flow vessel is
        // preferred. Large trunks then persist across many bifurcations while
        // shedding small branches, raising the Strahler diameter (RD) and
        // bifurcation (RB) ratios toward coronary morphometry.
        double my_sideBranchBias = 0.0;
    ...
    // Internal algorithm parameter
        ...
        // myExtensionMode: if true, skip stage 1 and use modified stage 2 constraints
        bool myExtensionMode = false;
        // myAnywhereMode: if true, allow connecting anywhere (no coronary restrictions)
        bool myAnywhereMode = false;

        // Track the most recently modified segment for incremental updates
        unsigned int myLastUpdatedSegment = 1;
...
public:
    ...
    HctccoModel(DomCtr &aDomCtr): myDomainController_(aDomCtr){}
    /**
    ...
    HctccoModel(double aPerf, unsigned int nTerm,  DomCtr &aDomCtr,
                       double aRadius = 1.0, bool extensionMode = false, bool anywhereMode = false): myDomainController_(aDomCtr) {
        assert(nTerm>=1);
        ...
        myExtensionMode = extensionMode;
        myAnywhereMode = anywhereMode;
        if(nTerm > 250)
            my_pTerm = 7.98e3;
        my_pDrop = my_pPerf-my_pTerm;
     ...
     */
    HctccoModel::Segment updateResistanceFromRoot(unsigned int segIndex=1);
    
    /**
     * Incrementally update resistances along the path impacted by a structural change.
     * @param segIndex index of the segment whose subtree changed
     */
    void updateResistanceIncremental(unsigned int segIndex);
    
    /**
     * Propagates a terminal flow increment from segIndex up to the root,
     * incrementing each ancestor's terminal count and accumulating its flow.
     * @param segIndex index of the segment whose subtree gained a terminal
     * @param flowDelta flow contributed by the new terminal (q_term * weight)
     */
    void updateFlow(unsigned int segIndex, double flowDelta);

    /** Backward-compatible overload: propagate one unit-weight terminal flow. */
    void updateFlow(unsigned int segIndex) { updateFlow(segIndex, my_qTerm); }

    /**
     * Deterministic per-terminal perfusion-flow weight (mean ~1). Derived from
     * the terminal location so the same point yields the same weight across
     * speculative candidate evaluations. Returns 1.0 when my_termFlowCV<=0.
     * @param p terminal location
     */
    double terminalFlowWeight(const TPointD &p) const;
    ...
    /**
     * Checks if a segment is eligible for connection (terminal or parent of terminal)
     * @param segIndex: segment index to check
     * @return true if segment is a valid connection point for growth
     */
    bool isConnectable(unsigned int segIndex) const;
    
    /**
     * Gets connectable segments for middle stage (leaf nodes + 3-6 generations above)
     * @return vector of segment indices that are connectable in middle stage
     */
    std::vector<unsigned int> getConnectableSegmentsForMiddleStage() const;
    
    /**
     * Validates if a new connection meets coronary-specific constraints
     * @param p: new point location
     * @param segIndex: target segment for connection
     * @param maxLength: maximum allowed length for new small branches (default 0.1mm)
     * @param minRadius: minimum radius for small coronary branches
     * @return true if connection meets constraints
     */
    bool isValidConnection(const TPointD &p, unsigned int segIndex) const;
                                  
    /**
     * Enhanced distance check for coronary networks with physiological constraints
     * @param p: point to check
     * @param minPerfusionDistance: minimum distance to maintain perfusion areas
     * @return true if point satisfies distance constraints
     */
    bool isDistanceValid(const TPointD &p, double minPerfusionDistance = 0.05) const;
    ...
    private:
    // Percentage-based progressive constraints.
    // Progress = terminals grown so far / target terminals (0..1).
    // (The previous formula was segments/myKTerm which saturates at ~2 immediately
    // and kept growth permanently in the late stage.)
    double getProgressPercentage() const {
        if (my_NTerm == 0) return 0.0;
        double p = static_cast<double>(myKTerm) / static_cast<double>(my_NTerm);
        if (p < 0.0) p = 0.0;
        if (p > 1.0) p = 1.0;
        return p;
    }
    
    bool shouldUseCoronaryRestrictions() const {
        double progress = getProgressPercentage();
        if (myExtensionMode) {
            return true; // Always use coronary restrictions in extension mode
        } else {
            return progress > 0.10; // Use coronary restrictions after 10% completion in normal mode
        }
    }
    
    double getConnectionRestrictionLevel() const {
        double progress = getProgressPercentage();
        if (myExtensionMode) {
            // Extension mode: Skip stage 1, start directly with modified stage 2
            if (progress < 0.25) return 0.5;     // 0-25%: Modified middle stage (leaf + 1-3 generations above)
            return 1.0; // 25-100%: Full coronary restrictions (leaf + immediate parent only)
        } else {
            // Normal mode: Three-stage approach
            if (progress < 0.10) return 0.0;      // 0-10%: No restrictions (any segment)
            if (progress < 0.35) return 0.5;     // 10-35%: Middle stage (leaf + 3-6 generations above)
            return 1.0; // 35-100%: Full coronary restrictions (leaf + immediate parent only)
        }
    }
    
    // Maximum branch length constraint: matches post-processing threshold.
    double getMaxBranchLength() const {
        return 25.0; // Same units as post-processing filter (prevents unrealistic long vessels)
    }
    
    double getMinBranchRadius() const {
        return 0.005; // Consistent minimum
    }
    
    Segment recomputeSegmentResistanceOnly(unsigned int segIndex);
    Segment applyResistanceUpdate(unsigned int segIndex,
                                  const Segment &sLeft,
...
/**
 * Overloads 'operator<<' for displaying objects of class 'HctccoModel'.
 * @param  out the output stream where the object is written.
 * @param aCoronaryTree the object of class 'HctccoModel' to write.
 * @return the output stream after the writing.
 */
template <typename DomCtr, int TDim>
std::ostream&
operator<< ( std::ostream & out, const HctccoModel<DomCtr, TDim> & aCoronaryTree );

template <typename DomCtr, int TDim>
bool
operator==(typename HctccoModel<DomCtr, TDim>::Segment  S1,typename  HctccoModel<DomCtr, TDim>::Segment S2);
